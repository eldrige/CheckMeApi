...
const updateOne = (Model, selectOptns) =>
  catchAsync(async (req, res, next) => {
...
const createOne = (Model) =>
  catchAsync(async (req, res, next) => {
    ...
    const newDocument = await Model.create({ ...req.body, author: userId });
    res.status(201).json({
      status: 'success',
      data: {
        newDocument,
...
const getOne = (Model, populateOptns, selectOptns) =>
  catchAsync(async (req, res, next) => {
    let query = Model.findById(req.params.id);
    if (populateOptns) query = query.populate(populateOptns);
    if (selectOptns) query = query.select(selectOptns);
